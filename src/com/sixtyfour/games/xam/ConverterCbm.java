...
public class ConverterCbm {
	...
	public static void main(String[] args) {
		ConverterCbm conv = new ConverterCbm();
		conv.convert();
	}
	...
	private void writeEndId(int endId) {
		try (OutputStream os = new FileOutputStream(new File("seqcbm/endid.def"))) {
	...
	private void convertCommands() {
		...
		try (Reader fr = getReader("xml/commands/commands.xml");
				OutputStream os = new FileOutputStream(new File("seqcbm/commands.def"))) {
	...
	private void convertItems() {
		...
		try (Reader fr = getReader("xml/items/items.xml");
				OutputStream os = new FileOutputStream(new File("seqcbm/items.def"))) {
			DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
			DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
			Document doc = dBuilder.parse(getInputSource(fr));
		...
		try (Reader fr = getReader("xml/items/items.xml");
				OutputStream os = new FileOutputStream(new File("seqcbm/operations.def"))) {
			DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
			DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
			Document doc = dBuilder.parse(getInputSource(fr));
	...
	public int convertRooms() {
		...
		for (File room : rooms) {
			...
			try (Reader fr = getReader(room);
					OutputStream os = new FileOutputStream(new File("seqcbm/" + name + ".rom"))) {
				DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
				DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
				Document doc = dBuilder.parse(getInputSource(fr));
	...
	private void write(OutputStream os, String prefix, String txt, boolean fullText) {
		try {
			...
			for (String part : parts) {
				...
				if (pc < parts.length && parts[pc].length() + len > 80) {
