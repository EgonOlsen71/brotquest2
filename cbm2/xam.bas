...
40010 if ex=0 then return
40020 if ex=1 then print "Das klappt so nicht!"
40030 if ex=3 then print "Das bewirkt nichts mehr!"
40040 if ex<>2 then ex=0:return
40050 ex=0
...
40510 tx%=0
40520 a$=id$(t%,tx%):if a$="***" then return
40530 gosub 59100:tx%=tx%+1:goto 40520

40600 rem print can-not-do-that-message
40605 ex=0: if t%=15 then print "Das will niemand haben!":return
...
43030 for i=0 to od%-1:if od$(i)=tf$ then ex=3:rt%=1:return
43040 next
...
45010 print "Moegliche Befehle:":print
...
50015 gosub 63000:input cc$:print
50020 ct$="":lc$="":if len(cc$)=0 then cc$="help"
50030 ex=0:tx$=cc$:gosub 63100:cc$=tx$+" ": rem add space to ease lexing
50035 gosub 41000
...
50150 next ii,p
50155 if t%<>-1 then if cv%(0)<>-1 then ex=1:return
...
52010 if ex<>0 or cc%=0 then return
...
60185 dim id$(51,4): rem item descriptions (mi%)
...
61430 id$(ii,t%)=tx$:t%=t%+1
61440 if a$="***" then return
61450 goto 61410
...
62005 gosub 62100:open2,dn%,2,rn$:gosub 61700:rd%=val(a$)
...
63005 i=peek(203):ii=peek(202)
63015 for p=53248 to 53327:poke p, 160:next
...
63250 rem convert
63260 if c%>191 then c%=c%-128:return
63270 if c%>90 then c%=c%-32:return
63280 return
